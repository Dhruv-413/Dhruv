...
|---|---|---|
| **0. Unblock** | Self-host `public/images/portrait.png`; per-route `opengraph-image.tsx` (typographic, no binary asset) and delete the dead `/og-image.jpg`, `/twitter-image.jpg` and unused `/images/projects/*` paths; `MotionConfig reducedMotion="user"` (ROADMAP §0); fix the title-doubling bug; delete dead code (`StatsSection.tsx`, unused `ScanLineEffect`) and the unused `recharts` dependency (removal, but confirm first) | Zero image 404s in the network panel; link preview renders in a debugger for `/` and `/github`; titles read "Page \| Dhruv Gupta" once |
