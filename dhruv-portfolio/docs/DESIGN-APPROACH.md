...
### 3.5 Slice 1 result (2026-09-20, uncommitted)
Built: INK/BONE tokens + Bricolage/JetBrains/Instrument Serif via `next/font` (light-mode attribute-selector hacks deleted); redesigned Header; Hero with a **3D voxel-relief portrait** (raw WebGL2, no library; see DESIGN.md §4.6), per-letter name reveal (CSS), magnetic primary CTA, scroll-linked drift; server-rendered work index; token-only footer. Deleted 11 legacy hero components and 4 data files, incl. the filler "Core Values" and the code-window furniture. Toasts are now lazy-loaded.
Verified: `tsc` 0 errors; `eslint` 0 errors (1 pre-existing warning); `next build` passes; browser checks at 375 / 768 / 1280 / 1440 / 1920, both themes, no horizontal overflow; reduced motion (final states at once, scroll tilt off); keyboard order and focus rings; JS-off HTML contains the name, portrait image, statement, facts and all five projects.
Measured (production build, this machine): hero canvas **154 fps idle, 109 fps at 4x CPU throttle**, p95 frame 12 ms; VoxelPortrait chunk ≈ 7.6 kB transferred.
**Lighthouse mobile does not meet the ≥ 95 target yet.** Simulated throttling, 3 runs each: first audit 67 median (LCP 4.0 s, TBT 0.9 s); after deferring WebGL init to idle + `fetchPriority="high"` on the portrait: 82-83; after also lazy-loading the toaster: 77-88 (TBT swings 140-410 ms because other processes were loading this machine, so the last two are within noise of each other). LCP stays ≈ 3.8 s simulated (element = the portrait `<img>`; ≈ 2.2 s of it is render delay, i.e. main-thread work before paint). Desktop profile: 99. CLS 0 everywhere. INP is unmeasured.
Where the remaining gap is (app-wide, mostly pre-existing, not the hero): ≈ 224 kB compressed first-load JS on every route (react-dom, framer-motion loaded on all routes incl. the `MotionConfig` I added in slice 0, TanStack Query, analytics), ≈ 1 s hydration long task under 4x CPU. Options, each needs your call: (a) migrate `motion.*` to `LazyMotion` + `m.*` across ~49 files (largest lever, broad refactor); (b) drop the Bricolage `opsz` axis (-36 kB font, visibly changes display cut) or `wdth` (-55 kB); (c) move TanStack Query out of the root layout to the routes that use it; (d) move `MotionConfig` out of the root so the home route skips framer-motion.
Review fixes applied after the three audits: reduced-motion leak in scroll tilt; idle loop now rests (WCAG 2.2.2); mobile menu trap includes the toggle and is no longer `aria-modal`; portrait no longer announced twice; list semantics and link names in the work index; 10 px low-contrast numerals removed; theme toggle 44 px and hairline-only; dark `--input` contrast fixed (documented value was wrong); tracking collisions fixed; hero dead space removed; mobile fold shows identity + all three actions.
Known and left: unresolved copy conflicts (Deloitte vs ONGC, project/certification counts) so no availability or internship claim beyond ONGC 2025 is made; the `/projects` links from the work index all go to `/projects` (per-project pages are slice 2); `next dev` caches `/_next/static` immutably because `next.config.ts` sets long-lived headers for dev too, so a long-running dev server serves stale chunks after big edits (restart it and hard-reload; not changed here).

### 3.6 Slices 2-4 result (2026-09-20, uncommitted)
Built: `/projects` (filterable hairline index) and `/projects/[id]` case studies (seeded cover, metrics, tech, links, code excerpt, prev/next, JSON-LD, per-project OG images); `/skills` (matrix + certifications, no proficiency numbers); `/career` (ledger); `/github` (heatmap in the cell language, visible at every width, readouts, languages, repositories, loading / error / empty states); `/contact` (restyled in place: same fields, validation, EmailJS payload and toasts; `watch()` replaced by `useWatch`); footer, 404, every `loading.tsx`, and a transform-only route-enter transition. Shared primitives: `src/components/ui/page-primitives.tsx` (`PageHeader`, `SectionHead`, `Tag`, `ArrowLink`) and `PageSkeleton`.
Removed: all legacy page components, `AnimatedBackground`, `StatCard`, `SectionHeader`, `ScrollReveal`, project modal/cards/filters, GitHub section components, unused colour helpers, the root `MotionConfig`. framer-motion now appears only in the lazy 3D chunk.
Verified: `tsc` 0 errors; `eslint` 0 errors and 0 warnings; `next build` passes (29 static pages); each route serves exactly one `<h1>` and one `og:image` in JS-off HTML; JSON-LD stays `<`-escaped; the sitemap lists the real project URLs; browser checks at 1440 dark and 375 light on every route, including contact validation errors; no horizontal overflow anywhere.
Lighthouse mobile (simulated, this machine, single runs unless noted), performance / accessibility: `/` 85-90 / 100 (median of 3), `/projects` 90 / 100, `/skills` 91 / 100, `/career` 91 / 100, `/contact` 89 / 100, `/projects/stock-analysis` 85 / 100, `/github` 82 / 100 (loading state; the real API is not reachable locally). CLS 0 on all. **No route reaches the ≥ 95 target.** Every route shows the same simulated LCP floor of about 3.3-3.9 s, which points at app-wide cost (roughly 220 kB of first-load JS and 130 kB of Bricolage with the `opsz` + `wdth` axes), not at any one page. The options are the ones in §3.5 (drop the `opsz` axis, move TanStack Query and the toaster out of the root, trim the analytics scripts); they need the owner's call because the font one changes the look.
Also removed this round: the unused shadcn `card`, `input`, `textarea`, `select` and `label` primitives (their Radix packages are now unused dependencies in `package.json`, left untouched), and the scroll listener in `PortraitStage` (touch scrolling already fires `touchstart`).
Not verified: a real EmailJS send (dummy keys locally); the `/github` success state was only checked with mocked API data (the real API needs the token); light theme at 1440 for the new routes (checked at 375); contrast of `text-destructive` error text was eyeballed, not computed.
Data issues surfaced by rendering the owner's JSON (not changed): the Deloitte entry still reads "Upcoming Virtual Intern" with a Jan 2026 start; some project `metrics` show "Login Success 100%"; `NEXT_PUBLIC_SITE_URL` resolves to a placeholder host in the built sitemap and OG URLs.
